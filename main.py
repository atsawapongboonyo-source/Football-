...
@app.get("/health")
def health():
    return {"status":"ok","version":VERSION,"frontend_expected":VERSION,"deployment_marker":"match-intelligence-044"}

@app.get("/api/version")
def api_version():
    return {"backend":VERSION,"frontend_expected":VERSION,"deployment_marker":"match-intelligence-044","javascript_mode":"inline"}
...
def model_performance():
    ...
    if not resolved:
        return {"resolved_predictions":0,"pending_predictions":len(rows),"note":"ยังไม่มีคำทำนายก่อนแข่งที่มีผลจริงสำหรับคำนวณสถิติ"}
    n=len(resolved)
    brier=[]
    for r in resolved:
        a=r["actual"]; hg,ag=a["home_goals"],a["away_goals"]
    ...
    return {
        ...
        "note":"นับเฉพาะ prediction ที่บันทึกก่อนแข่งและผูกกับโปรแกรมแข่งขันจริง",
    }
...
def predict(req: PredictRequest):
    ...
    try:
        ...
        if context.get("status")=="upcoming" and context.get("fixture"):
            saved=save_prediction(result,fixture_date=context["fixture"]["date"])
