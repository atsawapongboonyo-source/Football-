...
app = FastAPI(title="Fooball", version="0.3.0")
...
def health():
    return {
        "status": "ok",
        "version": "0.3.0",
        "model": "recency-weighted Poisson + Dixon-Coles low-score adjustment + Elo",
        "data_mode": predictor.data_mode,
    }
...
def predict(req: PredictRequest):
    if req.home_team == req.away_team:
        raise HTTPException(status_code=400, detail="กรุณาเลือกคนละทีม")
    if req.home_team not in predictor.current_teams or req.away_team not in predictor.current_teams:
        raise HTTPException(status_code=400, detail="ไม่พบทีมใน Premier League 2026/27")
    try:
